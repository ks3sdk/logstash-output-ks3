...
require "logstash/outputs/ks3/file_generator"
      ...
      class FileManager
          ...
          prefixes.each do |prefix|
            get_file_generator(prefix) do |file_generator|
              ...
              if file.size == 0 && Time.now - file.ctime > STALE_FILES_CHECK_INTERVAL_IN_SECONDS
                @logger.info("Logstash KS3 Output Plugin starts to remove stale file",
                             :key => file.key,
                             :path => file.path,
                             :size => file.size,
        ...
        def start_stale_files_check
          @stale_check = Concurrent::TimerTask.new(:execution_interval => STALE_FILES_CHECK_INTERVAL_IN_SECONDS) do
            @logger.info("Logstash KS3 Output Plugin: start to check stale files")
            remove_stale_files
          end
